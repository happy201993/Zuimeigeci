...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        self.sections = @[ //测试条目，实际以歌词词条cell 填充
                          @[
                              @"Song 1",
                              @"Song 2",
                              @"Song 3",
...
- (void)viewDidLoad {
    [super viewDidLoad];
//    BmobObject* user = (id)[User getCurrentUser];
//    if (user == nil) {
...
//                
//                NSLog(@"%@",user);
//                User *curruser = [[User alloc] initFromBmobOjbect:user];
//            }
//            
//            
//        }];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
     ...
     SectionHeaderCell   *cell = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"sectionHeader" forIndexPath:indexPath];
        ...
        User* userxxx =(User*) [BmobUser getCurrentUser];
        
        
        User* userxxx11 = [[User alloc]initFromBmobOjbect:userxxx];
//        userxxx11.avatar
        [header.icon sd_setImageWithURL:[NSURL URLWithString:userxxx11.avatar]];
        [header.backgroundIcon sd_setImageWithURL:[NSURL URLWithString:userxxx11.avatar]];
        
        //        User* user =(id) [BmobUser getCurrentUser];
//        [header.icon sd_setImageWithURL:user.avatar];
        return cell;
    }
    return nil;
}
